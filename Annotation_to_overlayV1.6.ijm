...
// Version 1.6 (9 August 2024)
// - Sorted the problem that flatten image resulted sometiems in all annotations on all slices by replacing
//   Flatten by Draw
...
macro Annotation_to_overlay{
	...
	if (Fl == true){ 
		...
		if (is("Virtual Stack")) run("Duplicate...", "duplicate"); // Cannot draw on virtual stack
		for (i = 0; i < roiManager("count"); i++) {
		roiManager("select", i);
		C = Roi.getStrokeColor();
		if (i==0) C1 = C; // Check if ROI colour has changed
		if (C != C1){
			if (C == "red") setForegroundColor(255, 0, 0);
				else {
				if (C == "green") setForegroundColor(0, 255, 0);
					else {
					if (C == "blue") setForegroundColor(0, 0, 255);
						else {
						if (C == "yellow") setForegroundColor(255, 255, 0);
							else {
							if (C == "cyan") setForegroundColor(0, 255, 255);
								else {
								if (C == "magenta") setForegroundColor(255, 0, 255);
									else {
									if (C == "white") setForegroundColor(255, 255, 255);
										else {
										if (C == "black") setForegroundColor(0, 0, 0);
									}
								}
	...
	// make it easier to see the macro is finshed 
	run("Select None");
	setSlice(1);
	setBatchMode("show");
